...
nodemon是一个命令行工具，实施重启服务器。用nodemon命令替代node命令执行文件
...
```js
 ...
 // 将特定模板与特定数据进行拼接
 const html = template('./views/index.art',{data: {age: 20}}); 
```
...
携带HTML标签，默认模板引擎不会解析标签，会将其转义后输出
...
- 项目根目录下新建config文件夹
- 在config文件夹下面新建`default.json、development.json、production.json`文件
- 在项目中通过require方法，将模块进行导入
- 使用模块内部提供的get方法获取配置信息

**将敏感配置信息存储在环境变量中**

- 在config文件夹中建立`custom-environment-variables.json`文件
...
使用`npm install 包名`下载的文件会默认被添加到 package.json 文件的 dependencies 字段中

## 开发依赖

在项目的开发阶段需要依赖，线上运营阶段不需要依赖的第三方包，称为开发依赖

使用`npm install 包名 --save-dev`将包添加到package.json文件的devDependencies字段中
...
6.如果没有index.js查看该文件夹中的**package.json中的main选项确定模块入口文件**
...
- 异步函数默认**返回promise对象**
- 使用return关键字（替代了resolve方法）返回promise对象（结果包含其中），调用异步函数再链式调用then方法获取异步函数执行结果
- 使用throw关键字抛出程序异常，调用异步函数再链式调用catch方法获取异步函数执行的错误信息
